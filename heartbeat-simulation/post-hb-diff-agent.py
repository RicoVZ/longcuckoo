...
class Malware(MalwareActions):
    ...
    def init(self):
        ...
        self.other_params = [
            {"test": "234"},
            {"test1": "adaa", "test2": "89sdf"},
            {"test3": "tadaa", "test4": "8dsfsdfds", "logfile": "stuff.log"},
            {"test5": "tadaa", "test6": "8dsfsdfds", "test7": "89sfs8df", "test11": "9sdf89sdfu"},
            {"test8": "tadaa", "logfile": "textlog.log", "test9": "89sfs8df", "test10":"9asd8asd", "test12":"90a8sda"},
        ]

        self.files = ["reporter.bin", "file.txt", "other.js", "help.php", "index.html"]

    def actions(self):
        traffic = Traffic()
        traffic.rand_interval = True
        traffic.rand_min = 20
        traffic.rand_max = 200
        traffic.method = "POST"
        started = False
        count = 0
        for url in self.urls:
            traffic.url = url
            traffic.get_params = self.files[count]
            traffic.post_data = self.other_params[count]
